...
$\qquad$ $\mu_{1} = \eta_{1}\boldsymbol{x}_{-(1,M)}$
...
$\qquad$ $\mu_{M} = \eta_{M}\boldsymbol{x}_{-(a,b)}$
...
Return samples $\boldsymbol{x}_{1},...,\boldsymbol{x}_{(a,b)},...$
...
Below we implement the MCMC-block-algorithm as descrived in the text. Note that we also store the count of accepted proposals in the algorithm.
```{r}
